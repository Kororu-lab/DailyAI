...
async def collect_news():
        ...
        try:
            ...
            for news in news_list:
                try:
                    ...
                    if cutoff_date <= news_date <= yesterday:
                        all_news.append(news)
                except ValueError:
                    # 날짜 형식이 다른 경우 어제 날짜로 설정
                    news['date'] = yesterday.strftime("%Y-%m-%d")
                    all_news.append(news)
    ...
    output_file = data_dir / f"ai_news_{yesterday.strftime('%Y%m%d')}.json"
